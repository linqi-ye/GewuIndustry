...
public class G1opAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        for (int i = 15; i < ActionNum; i++) SetJDeg(acts[i], ang[i-15]*180f/3.14f, 2000f, 200f);
        //float[] grasp=new float[12] {40,40,40,40,  -40,-40,   -40,-40,-40,-40,  40,40};
        float[] grasp=new float[12] {40,0,40,0,  -40,-0,   -50,-0,-50,-0,  50,0};
        ...
        if(Mathf.Abs(hand[6].jointPosition[0])>0.1f)g1ik.untrack();
        else g1ik.start_track();
        //g1ik.untrack();
        ...
        Vector3 totar = tar.position - body.position;
        totar.y = 0; 
        Vector3 robotForward = body.forward;
        robotForward.y = 0;
        float angleDiff = Vector3.SignedAngle(robotForward.normalized, totar.normalized, Vector3.up);
        //if(totar.magnitude>0.2f)wr = Mathf.Clamp(angleDiff * 0.02f, -0.5f, 0.5f);

        if(Mathf.Abs(angleDiff)<2)
        {
            wr=0;
            //vz=Mathf.Clamp(totar.magnitude * 0.5f, -1f, 1f);
        }
        wr=0;
        vz=0;
        if(Time.time>7.5f && Time.time<10.9f)wr=0.5f;
        if(Time.time>11.1f && Time.time<12.5f)vz=0.8f;
    }
